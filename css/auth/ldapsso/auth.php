...
require_once($CFG->dirroot.'/auth/ldapsso/lib_ldapsso.php'); //contains IP check function.

/**
 * LDAP authentication plugin.
 */
class auth_plugin_ldapsso extends auth_plugin_base {

    /**
     * Constructor with initialisation.
     */
    function auth_plugin_ldapsso() {
        $this->authtype = 'ldapsso';
        $this->config = get_config('auth/ldapsso');
     ...
     /**
     * Will get called before the login takes place if LDAP SSO
     * Portal is valid, and the Portal SALT is correct redirect
     * to the main page for LDAP SSO...
     * Note:
     *   Your web server must be configured to create this variable. For example in Apache you'll 
     *   need HostnameLookups On inside httpd.conf for it to exist.
     *
     */
    function loginpage_hook($key) {
        global $CFG;

        if (!empty($this->config->ldapsso_auth_key)   // SSO SALT enabled
            && !empty($this->config->ldapsso_portal)  // have a valid Portal Configured
            && $this->access_key_validate($key)      // Required SALT Key
            && ($_SERVER['REMOTE_ADDR']==$this->config->ldapsso_portal || $_SERVER['REMOTE_HOST']==$this->config->ldapsso_portal || $_SERVER['SERVER_NAME']==$this->config->ldapsso_portal)
    ...
    /**
    * Can user create an account in Moodle if not exists ?
    */
    function can_create_account() {
      if($this->config->auth_user_create_internal)
         return true;
    ...
    /** 
    * Can user create and account in external auth LDAP Directory
    * if so what context should be used...
    */
    function can_signup() {
    ...
    /**
    * Can user create use email confirmation...
    */
    function can_confirm(){
     return true;
    }
    /**
     * Confirm the new user secret/username as registered.
     */
   ...
   // validate required Portal accessKey and domain for authentication
    function access_key_validate($key){
         if(!empty($key)){  // is $key variable valid ?
             if(strcmp($this->config->ldapsso_auth_key,$key)==0){
                 return true;
             }else{
                 error("LDAP SSO Login from Remote Portal denied - Invalid Key!");
             }
           }

        return false; // inforce implicit deny
    }
    ...
    function user_login($username, $password) {
       ...
       if (!$username) {    // Don't allow blank usernames  - allow Blank passwords as the oncampuslogin will pass a blank password.
         return false;
       }
        ...
        if ($ldapconnection) {
            ...
            // Now Try to bind with current username and password
    ...
    function user_create($userobject, $plainpass) {
        ...
        $firstname = $textlib->convert(stripslashes($userobject->firstname), 'utf-8', $this->config->ldapencoding);
        $lastname = $textlib->convert(stripslashes($userobject->lastname), 'utf-8', $this->config->ldapencoding);
        ...
        $objectClass = ereg_replace('objectClass=','',$this->config->objectclass);

        switch ($this->config->user_type)  {
            case 'edir':
                $newuser['objectClass']   =  array($objectClass, 'organizationalPerson','person','top');
                $newuser[$this->config->user_attribute]  = $extusername;
              ...
              case 'ad':
                $newuser['objectClass']   =  $objectClass;
                $newuser[$this->config->user_attribute] = $extusername;
                $newuser['userpassword']  = $extpassword;
                $newuser['sn'] = $lastname;
                break;
        ...
        $user_dn = $this->config->user_attribute.'='.$extusername.','.$this->config->create_context;
        //$newuser['dn'] = $user_dn;
        //print_object($newuser);
        $uadd = ldap_add($ldapconnection,$user_dn,$newuser);
        ldap_close($ldapconnection);
        return $uadd;
    ...
    function sync_users ($bulk_insert_records = 1000, $do_updates = true) {
        ...
        $origdebug = $CFG->debug;
        $CFG->debug = DEBUG_DEVELOPER;  // DEBUG_ALL, DEBUG_MINIMAL, DEBUG_DEVELOPER
        ...
        switch (strtolower($CFG->dbfamily)) {
            case 'mysql':
                $droptablesql[] = 'DROP TEMPORARY TABLE IF EXISTS '. $temptable; // sql command to drop the table (because session scope could be a problem)
        ...
        foreach ($contexts as $context) {
            ...
            if ($entry = ldap_first_entry($ldapconnection, $ldap_result)) {
                do {
                    ...
                    if(strpos($value[0],'$') && $this->config->user_type=='ad') // Eliminiate AD Service Accounts
                      continue; // Skip AD Service Account Entries
        ...
        if (!empty($this->config->removeuser) and $this->config->removeuser == 1) {
            ...
            if (!empty($revive_users)) {
                ...
                foreach ($revive_users as $user) {
                    ...
                    if (update_record('user', $updateuser)) {
                        echo "\t"; print_string('auth_ldap_sso_dbreviveuser', 'auth_ldapsso', array($user->username, $user->id)); echo "\n";
                    } else {
                        echo "\t"; print_string('auth_ldap_sso_dbreviveusererror', 'auth_ldapsso', $user->username); echo "\n";
                    }
                }
                commit_sql();
            } else {
...
/// User Updates - time-consuming (optional)
   ...
   /// Switch users that exist in extauth and Moodle that are currently using alternate login
        ...
        if (!empty($mauth_users)) {
            ...
            foreach ($mauth_users as $user) {
                ...
                if (!empty($user)) {
                    ...
                    echo "\t"; print 'Cannot switch $user->auth user : '.$user->username.' to LDAP auth!'; echo "\n";
                 }
            }
               commit_sql();
       ...
       /// User Additions
        ...
        if (!empty($add_users)) {
            ...
            foreach ($add_users as $user) {
                ...
                $username = $user->username;  // Allow . (dot char) in name
    ...
    /**
     * Bulk insert in SQL's temp table Skipping Duplicate Context Entries
    ...
    function ldap_bulk_insert($users, $temptable) {
      ...
      global $CFG;
       ...
       foreach ($users as $user){ // Iterate Users with DN

         if(!empty($user))
           if(!record_exists_sql("SELECT `username` FROM `".$temptable."` WHERE `username`='".addslashes($user)."'")){
                  $sqldata .="('". addslashes($user) ."',". $CFG->mnet_localhost_id ."),";
            }else{
                  $duplicates++;
                  echo "\t Skipped Duplicate Entry for username: ".$user." was already added! \n";
               }

       }

         print "\t+ " . (count($users)-$duplicates) . " users\n";
         
        // bulk insert -- superfast with $bulk_insert_records
       if(count($users)-$duplicates) {
         $sql = 'INSERT INTO `' . $temptable . '` (`username`, `mnethostid`)  VALUES '. rtrim($sqldata,"\x2C") .' ON DUPLICATE KEY UPDATE `username`=VALUES(`username`),`mnethostid`=VALUES(`mnethostid`)';
    ...
    function user_update($olduser, $newuser) {
        ...
        if (isset($olduser->auth) and $olduser->auth != 'ldapsso') {
    ...
    function change_password_url() {
        ...
        } else {  // Custom Internal Change Password Page
            return  $CFG->httpswwwroot."/auth/ldapsso/change_password.php";
        }
    }
    ...
    function sync_roles($user) {
        ...
        if ($roles = get_roles_with_capability('moodle/legacy:coursecreator', CAP_ALLOW)) {
            ...
            if ($iscreator) { // Following calls will not create duplicates
                role_assign($creatorrole->id, $user->id, 0, $systemcontext->id, 0, 0, 0, 'ldapsso');
            } else {
                //unassign only if previously assigned by this plugin!
                role_unassign($creatorrole->id, $user->id, 0, $systemcontext->id, 'ldapsso');
            }
        }
    }
    ...
    function process_config($config) {
        ...
        if (!isset($config->removeuser))
            {$config->removeuser = 0; }
        if (!isset($config->auth_user_create_internal))
            {$config->auth_user_create_internal = 1; }
        if (!isset($config->ldapsso_failed_login_url))
            {$config->ldapsso_failed_login_url = $CFG->wwwroot."/auth/ldapsso/failed_login.php";}
        if (!isset($config->{'ldapsso_portal'}))
            {$config->{'ldapsso_portal'} = $_SERVER['SERVER_NAME']; }
        if (!isset($config->{'ldapsso_auth_key'}))
            {$config->{'ldapsso_auth_key'} = md5('hash'.rand(111,9876543)); }

        // save settings
        set_config('host_url', $config->host_url, 'auth/ldapsso');
        set_config('ldapencoding', $config->ldapencoding, 'auth/ldapsso');
        set_config('host_url', $config->host_url, 'auth/ldapsso');
        set_config('contexts', $config->contexts, 'auth/ldapsso');
        set_config('user_type', $config->user_type, 'auth/ldapsso');
        set_config('user_attribute', $config->user_attribute, 'auth/ldapsso');
        set_config('search_sub', $config->search_sub, 'auth/ldapsso');
        set_config('opt_deref', $config->opt_deref, 'auth/ldapsso');
        set_config('preventpassindb', $config->preventpassindb, 'auth/ldapsso');
        set_config('bind_dn', trim($config->bind_dn), 'auth/ldapsso');
        set_config('bind_pw', trim($config->bind_pw), 'auth/ldapsso');
        set_config('version', $config->version, 'auth/ldapsso');
        set_config('objectclass', $config->objectclass, 'auth/ldapsso');
        set_config('memberattribute', $config->memberattribute, 'auth/ldapsso');
        set_config('memberattribute_isdn', $config->memberattribute_isdn, 'auth/ldapsso');
        set_config('creators', $config->creators, 'auth/ldapsso');
        set_config('create_context', $config->create_context, 'auth/ldapsso');
        set_config('expiration', $config->expiration, 'auth/ldapsso');
        set_config('expiration_warning', $config->expiration_warning, 'auth/ldapsso');
        set_config('expireattr', $config->expireattr, 'auth/ldapsso');
        set_config('gracelogins', $config->gracelogins, 'auth/ldapsso');
        set_config('graceattr', $config->graceattr, 'auth/ldapsso');
        set_config('auth_user_create', $config->auth_user_create, 'auth/ldapsso');
        set_config('forcechangepassword', $config->forcechangepassword, 'auth/ldapsso');
        set_config('stdchangepassword', $config->stdchangepassword, 'auth/ldapsso');
        set_config('passtype', $config->passtype, 'auth/ldapsso');
        set_config('changepasswordurl', $config->changepasswordurl, 'auth/ldapsso');
        set_config('removeuser', $config->removeuser, 'auth/ldapsso');
        set_config('auth_user_create_internal', $config->auth_user_create_internal, 'auth/ldapsso');
        set_config('ldapsso_failed_login_url', $config->ldapsso_failed_login_url, 'auth/ldapsso');
        set_config('ldapsso_portal', $config->ldapsso_portal, 'auth/ldapsso');
        set_config('ldapsso_auth_key', $config->ldapsso_auth_key, 'auth/ldapsso');
