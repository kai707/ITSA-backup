...
/// Check to see if groups are being used here
	...
	$email = $USER->email;
	//$displayname = $USER->lastname." ".$USER->firstname."(".$USER->username.")";
	...
	//$displayname=$urlencode($displayname);//iconv("UTF-8","UTF-8","辛文義");
	if((($dbtime - 900) <= $now ) && ($now <= ($dbtime + 7200)))
	{
		if (!isguestuser() and isloggedin()) {
		        ...
		        if ($PAGE->user_allowed_editing())
		        {
		    	    ...
		    	    //print $url;
		    	    link_to_dimdimpopup_window($url," ","點按這裏開始網路會議 <br/>Click here to Start Meeting", 500, 700, get_string('modulename', 'dimdim'));
	        	}
	        	else {
				    ...
				    //print $url;
					link_to_dimdimpopup_window($url, " ", "點按這裏參加會議 <br/>Click here to Join Meeting", 500, 700, get_string('modulename', 'dimdim'));
					echo "<br /><br />一旦老師開始會議後，您就可以參加了。<br />You can join the meeting once the teacher has started the meeting";
					}
	...
	$outputwithoutdaysTW = " $hours 小時 $minutes 分 $seconds 秒之後，";
	$outputTW = " $days 天 $hours 小時 $minutes 分 $seconds 秒之後，";
